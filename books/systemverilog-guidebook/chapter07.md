...
本章では、SystemVerilogにおけるサブルーチンである**タスク（task）**と**関数（function）**について詳しく解説する。ソフトウェアプログラミング言語における「関数」や「プロシージャ」に相当するこれらの機能は、コードの再利用性と可読性を大幅に向上させる。タスクと関数の違い、引数の受け渡し方法、そして再帰呼び出しにおける `automatic` 指定の重要性を学ぶ。

---

## 7.2 タスクと関数の違い

SystemVerilogでは、再利用可能なコードブロックを定義するために**タスク（task）**と**関数（function）**の2つの仕組みが用意されている。両者は似ているが、重要な違いがある。

### 7.2.1 関数（function）

関数は、**時間経過を含まない**処理を記述するために使用する。つまり、`#`（遅延）、`@`（イベント待ち）、`wait` などの時間制御文を関数内で使用することはできない。関数は必ず**戻り値を返す**ことができる（`void` 型の関数を除く）。
...
タスクは、**時間経過を含む**処理を記述できる。クロック待ちやイベント待ちを含むテストベンチの処理に広く使われる。タスクは関数とは異なり、戻り値を直接返す構文を持たないが、`output` 引数や `ref` 引数を通じて値を返すことは可能である。
...
![図7.1: タスクと関数の違い](/images/systemverilog-guidebook/ch07_task_vs_function.drawio.png)
...
関数は `function` キーワードで宣言する。戻り値の型を指定し、`return` 文で値を返す。

**リスト7.3: 戻り値のある関数の宣言と呼び出し**
...
関数名自体を変数として使い、値を代入する方法もある。これはVerilog時代からの構文である。

**リスト7.4: 暗黙の戻り値を使用した関数**
...
戻り値が不要な場合は `void` 型を指定する。void型関数はタスクに似ているが、時間経過を含められない点が異なる。
...
![図7.2: 関数の戻り値とvoid関数の使い分け](/images/systemverilog-guidebook/ch07_function_return.drawio.png)
...
タスクは `task` キーワードで宣言する。時間制御文を含められるため、テストベンチでのバス・プロトコルの実装に広く使われる。

**リスト7.6: タスクによるAXI-Lite書き込みトランザクション**
...
タスクは関数とは異なり、戻り値を持たない。値を呼び出し元に返す場合は、`output` 引数を使用する。ただし、タスクでも `return` 文を使用することは可能である。タスクにおける `return` 文は値を返すためではなく、**タスクの早期終了**（途中で処理を打ち切る）ために使用する。エラー条件の検出時などに、残りの処理をスキップして即座にタスクを終了させたい場合に便利である。

**リスト7.7: return文によるタスクの早期終了**
...
`output` 引数を使って値を返す例は以下のとおりである。

**リスト7.8: output引数による値の返却**
...
SystemVerilogでは、タスクと関数の引数に4つの方向（direction）を指定できる。これにより、データの流れを明確にし、意図しない副作用を防ぐ。

### 7.5.1 input（入力引数）

呼び出し側から値を受け取る。デフォルトの方向である。タスク/関数の開始時に値がコピーされ、内部で変更しても呼び出し元の値は変わらない。
...
タスク/関数から呼び出し元へ値を返す。タスク/関数の終了時に値がコピーされる。
...
呼び出し時と終了時の両方で値がコピーされる。入力としても出力としても機能する。
...
`ref` は引数を**参照渡し**（pass by reference）で受け取る。これはC++の参照に相当し、値のコピーを行わず元の変数を直接操作する。大きなデータ構造（配列やオブジェクトなど）を渡す場合にパフォーマンス上の利点がある。

**リスト7.12: ref引数による参照渡し**
...
**重要**: `ref` 引数は `automatic` なタスク/関数でのみ使用できる。また、変更を防ぎたい場合は `const ref` を指定する。

**リスト7.13: const refによる読み取り専用参照**
...
![図7.3: 引数の受け渡し方向](/images/systemverilog-guidebook/ch07_argument_directions.drawio.png)

![図7.4: 引数の参照渡し（ref）の概念](/images/systemverilog-guidebook/ch07_ref_argument.drawio.png)

---

## 7.6 デフォルト引数値

SystemVerilogでは、関数やタスクの引数にデフォルト値を設定できる。呼び出し時に引数を省略するとデフォルト値が使用される。
...
SystemVerilogのタスクと関数は、デフォルトでは**static**（静的）である。ただし、デフォルトのライフタイムはコンテキストによって異なる。**モジュール内で宣言されたサブルーチンはデフォルトで `static`** であるが、**クラス内で宣言されたサブルーチンはデフォルトで `automatic`** である。static なサブルーチンでは、ローカル変数がすべての呼び出しで共有される。これは再帰呼び出しやマルチスレッド環境で問題を引き起こす。

一方、**automatic** を指定すると、各呼び出しごとに独立したローカル変数領域が確保される。再帰呼び出しを行う場合は必ず `automatic` を指定する必要がある。

**リスト7.15: staticとautomaticの違い**
...
![図7.5: static変数とautomatic変数の違い](/images/systemverilog-guidebook/ch07_static_automatic.drawio.png)
...
関数は合成可能な記述にも使用でき、組み合わせ回路の見通しを良くする。

**リスト7.18: 組み合わせ回路での関数利用（ALU）**
...
テストベンチでは、バスプロトコルの操作を再利用可能なタスクとして定義することが一般的である。

**リスト7.19: テストベンチでのタスク活用例**
...
本章では、SystemVerilogのタスクと関数について学んだ。重要なポイントを振り返る。

1. **関数（function）** は時間経過を含まない処理に使い、戻り値を返せる。組み合わせ回路の記述にも利用可能。
2. **タスク（task）** は時間経過を含む処理に使い、テストベンチでのプロトコル操作に最適。
3. **引数の方向**（`input`, `output`, `inout`, `ref`）を適切に指定することで、データの流れを明確にする。
4. **`ref` 引数**は参照渡しであり、大きなデータ構造を効率的に扱える。`automatic` 必須。
5. **`automatic` 指定**は再帰呼び出しやマルチスレッド環境で必須である。staticでは変数が共有されるため、予期しない動作の原因となる。
6. **デフォルト引数**を活用することで、柔軟で使いやすいインターフェースを設計できる。

次章では、これらのタスクや関数、型定義をパッケージとしてまとめ、プロジェクト全体で共有する方法について学ぶ。
