...
本章では、SystemVerilog の**機能カバレッジ（Functional Coverage）**について解説する。ランダム検証では膨大な数のテストが自動生成されるが、「テストが十分に行われたか」「どのシナリオがまだ検証されていないか」を把握するのは容易ではない。機能カバレッジは、検証の**網羅性を数値化**し、検証のゴールを明確にする仕組みである。`covergroup`、`coverpoint`、`cross` の使い方と、カバレッジ目標の設定方法を学ぶ。
...
コードカバレッジが100%でも、仕様上テストすべきシナリオが抜けている可能性がある。機能カバレッジはエンジニアが「何を検証すべきか」を明示的に定義し、その達成率を追跡する。

### 13.2.2 機能カバレッジの流れ

1. **カバレッジモデルの定義**: `covergroup` で検証対象のシナリオを記述
2. **シミュレーションの実行**: テスト実行中にカバレッジ情報が自動収集される
3. **カバレッジの分析**: 達成率を確認し、未到達のシナリオを特定する
4. **テストの追加**: カバレッジ穴（coverage hole）を埋めるテストを追加する
5. **目標達成**: すべてのカバレッジが目標値に到達したら検証完了とする

![図13.1: 機能カバレッジの検証フロー](/images/systemverilog-guidebook/ch13_coverage_flow.drawio.png)
...
`covergroup` は、カバレッジ収集の対象と条件を定義するブロックである。

**リスト13.1: covergroupの定義とインスタンス化**
...
![図13.2: covergroupの構造](/images/systemverilog-guidebook/ch13_covergroup_structure.drawio.png)

### 13.3.2 サンプリングのタイミング

カバレッジデータは、指定されたタイミングで「サンプリング」される。サンプリングの方法は以下のとおりである。

**リスト13.2: サンプリングのタイミング指定方法**
...
`coverpoint` に明示的なビン定義がない場合、ツールが自動的にビンを生成する。

**リスト13.3: 自動ビンとオプション設定**
...
実際のプロジェクトでは、意味のあるビンを明示的に定義することが推奨される。

**リスト13.4: 明示的なビン定義と遷移ビン**
...
![図13.3: coverpointのビン分割](/images/systemverilog-guidebook/ch13_coverpoint_bins.drawio.png)

### 13.4.3 ビンの配列

`[]` を使うと、各値に個別のビンが自動的に作成される。
...
`cross` は複数の `coverpoint` の組み合わせ（直積）をカバレッジとして定義する。個々の `coverpoint` だけでは検出できない、条件の組み合わせの網羅性を確認できる。
...
![図13.4: クロスカバレッジの2次元マトリックス](/images/systemverilog-guidebook/ch13_cross_coverage.drawio.png)

### 13.5.2 クロスカバレッジのフィルタリング

すべての組み合わせが意味を持つわけではない。`binsof` と `ignore_bins` でフィルタリングできる。

**リスト13.7: クロスカバレッジのフィルタリング**
...
![図13.5: カバレッジオプションの設定](/images/systemverilog-guidebook/ch13_coverage_options.drawio.png)
...
`type_option` は、同一型のすべてのインスタンスに共通するオプションを設定する。

**リスト13.9: type_optionによる共通オプション設定**
...
多くのシミュレータは、カバレッジデータベースをGUIで確認する機能を提供している。

**リスト13.11: シミュレータでのカバレッジレポート生成**
...
カバレッジが目標に達するまでテストを自動的に継続する手法である。

**リスト13.13: カバレッジ駆動検証の実装例**
...
本章では、SystemVerilogの機能カバレッジについて解説した。

1. **機能カバレッジ**はエンジニアが定義する検証の網羅性指標である。コードカバレッジとは補完的な関係にある。
2. **`covergroup`** でカバレッジモデルを定義し、`sample()` またはイベントでサンプリングする。
3. **`coverpoint`** で個別の変数のカバレッジを定義する。ビン（`bins`）で値を分類し、遷移ビンで状態遷移も検証できる。
4. **`cross`** で複数 `coverpoint` の組み合わせカバレッジを定義する。`ignore_bins` で不要な組み合わせを除外する。
5. **`option.at_least`** で各ビンの最小ヒット数、**`option.goal`** でカバレッジ目標を設定できる。
6. **カバレッジ駆動検証**により、カバレッジが目標に達するまで自動的にテストを継続する手法が実現できる。

次章からは第5部「高度なシステム連携編」に入り、C/C++との連携を可能にするDPI-Cについて学ぶ。
