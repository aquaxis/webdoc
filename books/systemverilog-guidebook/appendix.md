...
SystemVerilog IEEE 1800 規格で定義されている予約語（キーワード）の一覧を以下に示す。これらの識別子はユーザ定義の変数名やモジュール名として使用することはできない。
...
SystemVerilog で使用可能な主要なシステムタスクおよびシステム関数をカテゴリ別に整理する。システムタスクは `$` で始まる名前を持ち、シミュレータが提供する組み込み機能である。
...
SystemVerilog の演算子を優先度の高い順に以下の表に示す。優先度が同じ演算子はグループとしてまとめている。結合規則は、同じ優先度の演算子が複数並んだ場合にどちらから評価するかを示すものである。
...
SystemVerilog で「こういうことをしたいが、どう書けばよいか」という疑問に対して、目的から構文や機能を引けるように整理した。各項目は「やりたいこと」と「使用する構文・機能」の対応を示すものである。
...
> 本付録は SystemVerilog IEEE Std 1800-2017 に基づいて作成している。シミュレータや合成ツールによっては、一部の機能がサポートされていない場合がある。実際の使用時には各ツールのドキュメントも合わせて参照されたい。
