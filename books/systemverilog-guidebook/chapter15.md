...
本章では、SystemVerilogの**VPI（Verilog Procedural Interface）**と、設計・検証における**デバッグ技法**について包括的に解説する。VPIは、C言語からシミュレーション内部のオブジェクトにアクセスし、値の読み書きやコールバックの登録を可能にする強力なインターフェースである。カスタムシステムタスクの作成、信号値のモニタリング、ネットリストの走査など、VPIを活用した高度なシミュレーション制御の方法を学ぶ。
...
**VPI（Verilog Procedural Interface）**は、IEEE 1364-2001で標準化されたC言語プログラミングインターフェースである。VPIを使うことで、C/C++プログラムからシミュレーション内部のあらゆるオブジェクト（モジュール、ネット、レジスタ、ポート、タスク、関数など）にアクセスし、操作することができる。

VPIの主な用途は以下の通りである。

- **カスタムシステムタスク/関数の作成**: `$mytask` のような独自のシステムタスクを定義できる
- **シミュレーションの監視・制御**: 信号値の読み書き、コールバックによるイベント通知を実現する
- **外部ツールとの連携**: デバッガ、波形ビューワ、スクリプト言語との統合が可能である
- **ネットリストの走査**: 設計階層を動的にトラバースし、構造情報を取得できる
- **カバレッジ収集・解析**: カスタムカバレッジメトリクスを定義・収集できる

VPIはC言語のヘッダファイル `vpi_user.h` として提供され、シミュレータとリンクする共有ライブラリ（`.so` / `.dll`）として実装する。
...
VPIは長い歴史の中で進化してきた。その経緯を理解することは、既存コードの保守やツール選択に役立つ。

**表15.1: PLI 1.0からVPIへの歴史**
...
PLI 1.0の問題点は、TFとACCが別々に設計されたため一貫性がなく、機能の重複や制限があった。VPIはこれらを統一した単一のAPIとして再設計され、以下の改善を実現した。

- **統一されたオブジェクトモデル**: すべてのオブジェクトを `vpiHandle` で統一的に扱える
- **一貫したAPI命名規則**: すべての関数が `vpi_` プレフィックスを持つ
- **イテレータベースのアクセス**: オブジェクト群を効率的に走査できる
- **コールバックメカニズム**: イベント駆動型のプログラミングが可能である

### 15.2.3 VPIのオブジェクトモデル

VPIはオブジェクト指向のモデルを採用しており、シミュレーション内部のすべてのエンティティを**オブジェクト**として表現する。オブジェクト間の関係は**1対1（one-to-one）**と**1対多（one-to-many）**の2種類で表される。

![図15.1: VPIオブジェクトモデル](/images/systemverilog-guidebook/ch15_vpi_object_model.drawio.png)

主要なオブジェクトタイプは以下の通りである（表15.2）。
...
オブジェクトへのアクセスは `vpiHandle`（不透明なポインタ型）を通じて行う。

**リスト15.2: VPIオブジェクトモデルの走査例**
...
SystemVerilogでは、C言語との連携方法としてVPIの他に**DPI-C（Direct Programming Interface for C）**も提供されている。両者は目的と機能が異なるため、適切に使い分けることが重要である。

**表15.3: DPI-CとVPIの使い分け**
...
実際のプロジェクトでは、両方を組み合わせて使うことも一般的である。たとえば、DPI-Cでリファレンスモデルを呼び出し、VPIでカスタムデバッグタスクを実装するといった構成が考えられる。
...
VPIでオブジェクトにアクセスするための基本関数は3つある。

**`vpi_handle`** — 1対1の関係でオブジェクトを取得する。

**リスト15.3: vpi_handleによるオブジェクト取得**
...
**`vpi_handle_by_name`** — 名前を指定してオブジェクトを取得する。

**リスト15.4: vpi_handle_by_nameによる名前検索**
...
**`vpi_iterate` / `vpi_scan`** — 1対多の関係でオブジェクト群を走査する。

**リスト15.5: vpi_iterate/vpi_scanによる走査**
...
イテレータの使用における重要な注意点がある。

- `vpi_scan` が `NULL` を返すと、イテレータは自動的に解放される
- ループを途中で抜ける場合は、`vpi_free_object(iter)` を呼んでイテレータを明示的に解放する必要がある
...
オブジェクトのプロパティを取得するための関数群を紹介する。

**`vpi_get`** — 整数プロパティを取得する。

**リスト15.7: vpi_getによるプロパティ取得**
...
**`vpi_get_str`** — 文字列プロパティを取得する。

**リスト15.8: vpi_get_strによる文字列取得**
...
**`vpi_get_value`** — 信号の現在値を取得する。

**リスト15.9: vpi_get_valueによる値取得**
...
**`vpi_put_value`** を使って信号に値を設定できる。値の設定モードにはいくつかの種類がある。

**リスト15.10: vpi_put_valueによる値設定**
...
VPIのコールバック機構を使うと、特定のイベントが発生した際にC関数を自動的に呼び出すことができる。コールバックは `vpi_register_cb` で登録する。

**リスト15.11: VPIコールバックの登録**
...
主要なコールバック理由（`reason`）は以下の通りである（表15.6）。
...
VPIの最も一般的な用途の一つが、カスタムシステムタスクの定義である。`vpi_register_systf` を使って独自の `$mytask` を登録できる。

**リスト15.12: カスタムシステムタスクの登録**
...
`s_vpi_systf_data` の主要フィールドは以下の通りである（表15.7）。

**表15.7: s_vpi_systf_dataの主要フィールド**
...
実用的なカスタムシステムタスクの例として、指定した信号のビット幅と現在値を表示する `$sig_info` タスクを作成する。

**リスト15.13: $sig_infoシステムタスクの実装**
...
SystemVerilog側での使用例は以下の通りである。

**リスト15.14: $sig_infoのSystemVerilog側での使用例**
...
VPIのコールバック機構を使って、特定の信号の値変化を監視するモニタを実装する。

**リスト15.15: 信号値モニタリングの実装**
...
VPIを使ってシミュレーションの制御を行うことができる。`vpi_control` 関数は、シミュレーションの停止や終了を要求する。

**リスト15.16: シミュレーション制御の実装**
...
また、シミュレーション時刻の取得も重要な機能である。

**リスト15.17: シミュレーション時刻の取得**
...
![図15.2: VPIシミュレーション制御API](/images/systemverilog-guidebook/ch15_vpi_control.drawio.png)

### 15.4.4 ネットリストの走査

VPIの強力な機能の一つが、設計階層全体を動的に走査できることである。以下は完全な設計階層ダンプの実装例である。

**リスト15.18: ネットリストの走査と階層ダンプ**
...
VPIアプリケーションをシミュレータに登録するには、`vlog_startup_routines` 配列を定義する。この配列は、シミュレータがVPIライブラリをロードする際に自動的に呼び出される。

![図15.3: VPIコールバックフロー](/images/systemverilog-guidebook/ch15_vpi_callback.drawio.png)

**リスト15.19: VPIアプリケーションの登録**
...
コンパイルとシミュレータへの登録方法はシミュレータによって異なる。主要なシミュレータでの手順は以下の通りである。

**リスト15.20: VCS（Synopsys）でのコンパイル**
...
**リスト15.21: Questa/ModelSim（Siemens EDA）でのコンパイル**
...
**リスト15.22: Xcelium（Cadence）でのコンパイル**
...
![図15.4: VPIデバッグ戦略](/images/systemverilog-guidebook/ch15_debug_strategy.drawio.png)

![図15.5: VPI波形デバッグ](/images/systemverilog-guidebook/ch15_waveform_debug.drawio.png)
