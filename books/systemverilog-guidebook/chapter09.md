---
title: "第 9 章：オブジェクト指向プログラミング（OOP）"
---
...
本章では、SystemVerilogにおける**オブジェクト指向プログラミング（OOP）**の仕組みを解説する。クラスの定義、プロパティとメソッド、コンストラクタ（`new`）、継承（`extends`）、多態性（`virtual`）、抽象クラス、静的メンバなど、検証環境の構築に不可欠なOOP機能を体系的に学ぶ。OOPの概念はUVMをはじめとする現代の検証メソドロジの基盤であり、SystemVerilogエンジニアにとって必須の知識である。
...
**クラス（class）**はデータ（プロパティ）と操作（メソッド）をひとまとめにした設計図である。クラスから生成された実体を**オブジェクト（object）**またはインスタンスと呼ぶ。

ハードウェア記述における `module` がポートと内部回路の設計図であるのと同様に、`class` はデータ構造とその振る舞いの設計図である。ただし、クラスは検証（シミュレーション）専用であり、合成対象にはならない。
...
クラスのオブジェクトを使用するには、まず**ハンドル**（参照変数）を宣言し、`new` でオブジェクトを生成する。ハンドルの宣言だけではオブジェクトは生成されず、`null` 状態である。
...
`new` 関数はクラスの**コンストラクタ**である。オブジェクト生成時に自動的に呼び出され、プロパティの初期化を行う。明示的に定義しない場合、引数なしのデフォルトコンストラクタが自動的に提供される。
...
ハンドルの代入はオブジェクトのコピーではなく、**参照のコピー**である。2つのハンドルが同じオブジェクトを指すことになる。

**リスト9.4: ハンドルの代入とシャローコピー**
...
![図9.1: ハンドルとオブジェクトの関係](/images/systemverilog-guidebook/ch09_handle_object.drawio.png)

![図9.2: クラスの構造（プロパティとメソッド）](/images/systemverilog-guidebook/ch09_class_structure.drawio.png)
...
**継承**は、既存のクラス（基底クラス/親クラス）の機能を引き継いで新しいクラス（派生クラス/子クラス）を作る仕組みである。`extends` キーワードを使用する。
...
`super` は親クラスのメンバにアクセスするためのキーワードである。特にコンストラクタ内で親クラスの初期化を行う場合に重要である。

**リスト9.6: superによる親クラスへのアクセス**
...
`this` は現在のオブジェクト自身への参照である。プロパティ名と引数名が同じ場合の曖昧さ解消などに用いる。
...
![図9.3: 継承（Inheritance）の概念](/images/systemverilog-guidebook/ch09_inheritance.drawio.png)
...
**多態性**は、同じインターフェース（メソッド名）で異なる動作を実現する仕組みである。SystemVerilogでは、`virtual` キーワードを付けたメソッドが多態性をサポートする。

`virtual` を付けたメソッドは、実行時にオブジェクトの**実際の型**に基づいて呼び出される（動的ディスパッチ）。`virtual` なしの場合は、ハンドルの**宣言型**に基づいて呼び出される（静的ディスパッチ）。

**リスト9.8: virtualメソッドによる多態性**
...
`virtual` を付けないと、ハンドルの宣言型に基づいてメソッドが解決される。

**リスト9.9: virtualなしの場合の静的ディスパッチ**
...
**重要**: オーバーライドを意図するメソッドには必ず `virtual` を付けること。UVM環境では全メソッドが `virtual` であることが前提となっている。

![図9.4: 多態性の仕組み](/images/systemverilog-guidebook/ch09_polymorphism.drawio.png)

![図9.5: カプセル化とアクセス制御](/images/systemverilog-guidebook/ch09_encapsulation.drawio.png)
...
**抽象クラス**は `virtual class` として宣言し、直接インスタンス化できないクラスである。インターフェース（API）を定義し、具体的な実装はサブクラスに委ねる。
...
`pure virtual` として宣言されたメソッドは、本体を持たない。すべてのサブクラスで**必ず実装**しなければならない。これにより、サブクラスが必要なインターフェースを確実に実装することを保証する。
...
`static` プロパティは、クラスの全オブジェクトで**共有**される変数である。オブジェクトごとに個別の値を持つ通常のプロパティとは異なり、クラスに対して1つだけ存在する。
...
`static` メソッドは、特定のオブジェクトに紐付かない関数である。クラス名を使って直接呼び出すことができ、インスタンスを生成する必要がない。ただし、`static` メソッド内からは非 `static` メンバにアクセスできない。
...
静的メンバを活用した代表的なデザインパターンがシングルトンである。システム全体で1つだけ存在すべきオブジェクト（設定マネージャなど）に使用する。

**リスト9.15: シングルトンパターンの実装**
...
継承関係にあるクラス間で型変換を行う場合、`$cast` を使用する。

**リスト9.16: $castによる型キャスト**
...
本章では、SystemVerilogのオブジェクト指向プログラミング機能について学んだ。

1. **クラス**はデータ（プロパティ）と操作（メソッド）をカプセル化する設計図である。`new` でオブジェクトを生成する。
2. **継承（extends）**により既存クラスを拡張できる。`super` で親クラスにアクセスし、`this` で自身を参照する。
3. **多態性（virtual）**により、基底クラスのハンドルから派生クラスのメソッドを呼び出せる。検証環境の柔軟性に不可欠である。
4. **抽象クラス（virtual class）**と**純粋仮想関数（pure virtual）**で、サブクラスに実装を強制するインターフェースを定義できる。
5. **静的メンバ（static）**はクラス全体で共有されるプロパティ・メソッドである。ユーティリティ関数やカウンタに有用である。
6. **$cast** で安全な型変換を行う。ダウンキャストの成否を実行時にチェックできる。

次章では、OOPを基盤としたSystemVerilog独自の強力な機能――制約付きランダム検証について学ぶ。
