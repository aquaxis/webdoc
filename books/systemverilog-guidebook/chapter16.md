...
デバッグはVLSI設計フローにおいて最も多くの時間を費やす工程の一つである。本章では、$display系タスクによるテキストベースのデバッグから、波形解析、アサーションベースのデバッグ、UVMのレポーティング機能まで、体系的なデバッグ手法を解説する。効率的なデバッグ環境の構築方法についても学ぶ。
...
SystemVerilogの `$display` 系タスクは、最も基本的でありながら最も頻繁に使用されるデバッグ手段である。各タスクの特性を理解し、適切に使い分けることが重要である。

**リスト16.1: $display系タスクの基本**
...
`$timeformat` でタイムスタンプの表示形式をカスタマイズできる。

**リスト16.2: $timeformatによる時刻表示のカスタマイズ**
...
`$monitor` と `$strobe` は、`$display` とは異なるタイミングで実行される特殊な表示タスクである。

**リスト16.3: $monitorと$strobeの動作比較**
...
`$display` と `$strobe` の違いは重要である。

**リスト16.4: $displayと$strobeの実行タイミングの違い**
...
大規模なシミュレーションでは、コンソール出力だけでなくファイルにログを記録することが不可欠である。
...
マルチチャネルディスクリプタ（MCD）を使えば、複数の出力先に同時にログを書き出すこともできる。

**リスト16.6: マルチチャネルディスクリプタによる同時出力**
...
大規模プロジェクトでは、デバッグメッセージの出力レベルを制御する仕組みが重要である。

**リスト16.7: verbosityレベルによるデバッグメッセージ制御**
...
コンパイルディレクティブを使って、デバッグコードの有効/無効を切り替えることができる。

**リスト16.8: `ifdefによる条件付きデバッグコード**
...
コンパイル時にフラグを指定してデバッグモードを制御する。

**リスト16.9: コンパイル時のデバッグフラグ指定**
...
複数レベルのデバッグを条件付きコンパイルで制御するパターンも有用である。

**リスト16.10: 複数レベルのデバッグ制御**
...
**VCD（Value Change Dump）**は、IEEE 1364で標準化された波形データフォーマットである。ほぼすべてのシミュレータとビューワがサポートしている。

**リスト16.11: VCDファイルの出力と制御**
...
`$dumpvars` のレベル指定による制御は表16.2の通りである。

**表16.2: $dumpvarsのレベル指定**
...
**FSDB（Fast Signal Database）**は、Synopsys社が定義した高性能な波形フォーマットである。VCDに比べて圧縮率が高く、大規模設計での使用に適している。

**リスト16.12: FSDBファイルの出力**
...
波形ビューワは、信号の時間変化をグラフィカルに表示し、設計の動作を直感的に理解するための不可欠なツールである。

![図16.1: $display系タスクの使い分け](/images/systemverilog-guidebook/ch16_display_tasks.drawio.png)

主要な波形ビューワの特徴を表16.4にまとめる。
...
波形ビューワを効率的に使うためのポイントをまとめる。

1. **信号グループの活用**: 関連する信号をグループ化して保存しておく
2. **マーカーの設定**: 重要な時間点にマーカーを設定し、素早くジャンプできるようにする
3. **バスの基数設定**: アドレスやデータバスは16進数表示が見やすい
4. **アナログ表示**: カウンタやFIFOレベルなどはアナログ波形で傾向を把握する
5. **カーソル間の時間計測**: 2つのカーソルを使ってイベント間の時間を測定する

![図16.2: 波形解析のワークフロー](/images/systemverilog-guidebook/ch16_waveform_analysis.drawio.png)

### 16.2.4 波形解析のテクニック

波形デバッグを効率的に行うためのテクニックを紹介する。

**テクニック1: ブックマーク信号の追加**

特定のイベントを素早く特定するため、テストベンチにブックマーク用の信号を追加する。

**リスト16.13: ブックマーク信号によるデバッグ支援**
...
**リスト16.14: プロトコル信号のグループ化インターフェース**
...
波形ダンプはシミュレーション速度に大きな影響を与える。適切なダンプ範囲の制御が重要である。

**リスト16.15: ダンプ範囲の制御によるパフォーマンス最適化**
...
![図16.3: 波形ダンプとパフォーマンス](/images/systemverilog-guidebook/ch16_waveform_debug.drawio.png)
...
アサーションが失敗した場合の効果的なデバッグ手順を解説する。

**リスト16.16: アサーション失敗の解析例**
...
アサーション失敗のデバッグ手順は以下の通りである。

1. **失敗メッセージの確認**: 時刻、アサーション名、失敗原因を特定する
2. **波形での確認**: 失敗時刻付近の関連信号を波形ビューワで確認する
3. **アサーションの分解**: 複雑なアサーションは部品に分解して、どの部分が不成立かを特定する
4. **前提条件の確認**: `disable iff` や前提条件（antecedent）が正しいかを検証する
5. **修正と再テスト**: RTLまたはアサーションを修正し、再度テストを実行する

### 16.3.2 SVAデバッグのベストプラクティス

SVA（SystemVerilog Assertions）を効果的にデバッグするためのベストプラクティスを紹介する。

**リスト16.17: SVAデバッグのベストプラクティス**
...
`bind` 構文を使うと、RTLコードを一切変更せずにアサーションモジュールを挿入できる。これはデバッグにおいて非常に有用である。

**リスト16.18: bindによるアサーションの挿入**
...
- RTLソースコードに手を加える必要がない
- アサーションファイルをバージョン管理で別管理できる
- コンパイル時に `bind` ファイルを含めるかどうかで有効/無効を切り替えられる
- 複数のインスタンスに一括で適用できる

### 16.3.4 フォーマル検証との連携

アサーションはシミュレーションだけでなく、フォーマル検証ツールでも使用できる。

**リスト16.19: フォーマル検証対応のアサーション**
...
信号レベルのデバッグだけでなく、トランザクションレベルでの理解がバグの特定を大幅に効率化する。

**リスト16.20: トランザクション記録クラス**
...
UVM（Universal Verification Methodology）には、強力なデバッグ・レポーティング機能が組み込まれている。

**リスト16.21: UVMレポーティングマクロの使用**
...
UVMのverbosityレベルは表16.6の通りである。

**表16.6: UVMのverbosityレベル**
...
verbosityの制御はコマンドラインから行える。

**リスト16.22: UVM verbosityの制御**
...
UVMのレポートキャッチャーを使ったカスタムフィルタリングも強力である。

**リスト16.23: UVMレポートキャッチャーによるカスタムフィルタリング**
...
SystemVerilogのクラスベース検証環境では、メモリリークが問題になることがある。

**リスト16.24: メモリ使用量トラッカー**
...
メモリリークを防ぐためのベストプラクティスは以下の通りである。

- 使い終わったオブジェクトへの参照を `null` にして、ガベージコレクションを促進する
- キューや連想配列に蓄積されるオブジェクトのサイズを定期的に監視する
- `uvm_pool` や `uvm_queue` の使用時は、不要なエントリを削除する
- スコーピングを意識し、ローカル変数のオブジェクトはスコープ外で自動解放されることを利用する

### 16.4.4 パフォーマンスプロファイリング

シミュレーション速度の最適化は大規模プロジェクトにおいて重要である。

**リスト16.25: シミュレーション性能計測モジュール**
...
パフォーマンスのボトルネックとなる一般的な原因を表16.7にまとめる。
...
![図16.4: パフォーマンス最適化フロー](/images/systemverilog-guidebook/ch16_performance_optimization.drawio.png)

### 16.4.5 自動回帰テストのデバッグ戦略

CI/CD環境での大量テスト実行時に、効率的にデバッグを進めるための戦略を解説する。

![図16.5: デバッグ戦略フロー](/images/systemverilog-guidebook/ch16_debug_strategy.drawio.png)
...
自動回帰テストのデバッグでは、以下の戦略が効果的である。

1. **失敗の分類**: エラーメッセージをパターンで分類し、同一原因の失敗をグループ化する
2. **最小再現セット**: 失敗を再現する最小限のテスト条件を特定する
3. **シード値管理**: ランダムテストのシード値を記録し、失敗の再現を可能にする
4. **差分解析**: 成功時と失敗時のログを比較して差分を特定する
5. **段階的な絞り込み**: verbosityを段階的に上げて、問題の箇所を特定する
...
効率的なデバッグのためには、統一されたビルド・実行環境が不可欠である。

**リスト16.27: デバッグ環境用Makefile**
...
シミュレーションログの自動解析は、大量のテスト結果を効率的に処理するために必要である。以下にPythonスクリプトの例を示す。
...
SystemVerilog側でも、ログ解析しやすい構造化出力を心がける。
...
継続的インテグレーション環境でのデバッグは、対話的なデバッグとは異なるアプローチが必要である。

**リスト16.30: CI/CD向けのテスト制御モジュール**
...
CI/CDでのデバッグワークフローは以下のようになる。

1. **初回実行**: 波形ダンプなしで高速実行する
2. **失敗検出**: テスト結果を自動解析し、失敗テストを特定する
3. **再実行**: 失敗テストのみ、波形ダンプ付きで再実行する
4. **アーティファクト保存**: 波形ファイル、ログファイルをCI/CDのアーティファクトとして保存する
5. **ローカルデバッグ**: 開発者がアーティファクトをダウンロードしてローカルで解析する

### 16.5.4 チーム開発でのデバッグ共有

チーム開発では、デバッグ情報の共有が生産性に大きく影響する。

**リスト16.31: チーム共有のデバッグ設定ファイル**
...
デバッグ共有のベストプラクティスを表16.8にまとめる。

**表16.8: デバッグ共有のベストプラクティス**

| 項目 | 推奨事項 |
|------|---------|
| デバッグマクロ | チーム共通のヘッダファイルに定義し、バージョン管理する |
| 波形設定 | 信号グループ設定をスクリプト化し、リポジトリに格納する |
| アサーション | `bind` ファイルとして独立管理し、段階的に追加する |
| ログフォーマット | 構造化フォーマット（CSV/JSON）を統一し、解析ツールを共有する |
| デバッグ手順書 | よくある問題とその対処法をWikiやドキュメントに記録する |
| VPIツール | 共有ライブラリとしてビルドし、チームで利用できるようにする |

---

## 16.6 まとめ

本章では、VPI（Verilog Procedural Interface）とSystemVerilogにおける各種デバッグ技法を学んだ。主要なポイントを振り返る。

**VPIについて**:
- VPIはC言語からシミュレーション内部にアクセスするための標準インターフェースである
- `vpiHandle` によるオブジェクトモデルと、一貫した `vpi_` プレフィックスのAPI体系を持つ
- カスタムシステムタスクの作成、信号モニタリング、設計階層の走査が可能である
- DPI-Cとは目的が異なり、シミュレーション制御にはVPIが適している

**デバッグ基本技法**:
- `$display`, `$monitor`, `$strobe` はそれぞれ異なるタイミングで動作するため、使い分けが重要である
- ファイルログ出力とverbosityレベルの制御により、効率的なデバッグが可能になる
- `ifdef` ディレクティブでデバッグコードの有効/無効を切り替えられる

**波形デバッグ**:
- VCDは標準フォーマットで広く対応しているが、大規模設計にはFSDBが適している
- ダンプ範囲の制御はシミュレーション速度に直結するため、適切な設定が必要である
- ブックマーク信号やデバッグ用インターフェースを活用して波形解析を効率化できる

**アサーションベースデバッグ**:
- SVAとbind構文を組み合わせることで、RTLを変更せずにアサーションを追加できる
- フォーマル検証ツールとの連携も視野に入れたアサーション記述が望ましい

**高度なデバッグ**:
- UVMのレポーティング機能は、大規模検証環境での標準的なデバッグ手段である
- CI/CDパイプラインでの自動テストには、構造化されたログ出力と再現性の確保が重要である
- チーム開発では、デバッグ設定やツールの共有が生産性向上の鍵となる

---

## 演習問題

**問題1**: VPIを使って、指定したモジュール内のすべてのレジスタ（`reg`/`logic`）の名前、ビット幅、現在値を一覧表示するシステムタスク `$list_regs(module_path)` を実装せよ。C言語のソースコード全体を記述し、`vlog_startup_routines` の定義も含めること。

**問題2**: 以下の `$display` と `$strobe` の出力結果を予測せよ。
...
**問題3**: 以下の仕様を持つ `sync_fifo` モジュールに対して、`bind` を使って挿入するアサーションモジュールを設計せよ。検証すべきプロパティは以下の通りである。
- FIFO満杯時の書き込みが発生しないこと
- FIFO空時の読み出しが発生しないこと
- 書き込みデータと読み出しデータの順序が保証されること（FIFO順序性）
- リセット後にFIFOが空状態になること

**問題4**: VCDダンプのパフォーマンス最適化について、以下の状況でどのようなダンプ設定が最適か、理由とともに説明せよ。
1. 初回デバッグ時（問題箇所が不明）
2. 問題箇所が特定のモジュール内と判明している場合
3. 問題が特定の時間範囲で発生することが分かっている場合
4. CI/CDパイプラインでの回帰テスト時

**問題5**: UVMの `uvm_report_catcher` を拡張して、以下の機能を持つカスタムレポートキャッチャーを実装せよ。
- 同一IDのエラーが10回以上繰り返された場合、以降のメッセージを抑制する
- 抑制されたメッセージの数をシミュレーション終了時にレポートする
- 特定のIDリスト（既知のバグ）に該当するエラーを警告にダウングレードする

**問題6**: チーム開発を想定し、以下の要件を満たすデバッグ環境を設計せよ。Makefileのターゲット構成、デバッグマクロの定義、ログ解析の仕組みを含めた全体アーキテクチャを記述すること。
