...
SystemVerilogは、ハードウェア設計と検証の両方で必要となる多彩な配列型を提供している。従来のVerilogでは固定サイズの配列しか扱えなかったが、SystemVerilogでは動的配列、連想配列、キューといった柔軟なデータ構造が利用可能になった。
...
これらのデータ構造を正しく使い分けることで、テストベンチの記述効率が飛躍的に向上し、複雑なデータ管理も簡潔に表現できるようになる。

![図4.1: 配列の種類と特徴の概要](/images/systemverilog-guidebook/ch04_array_overview.drawio.png)
...
静的配列は、コンパイル時にサイズが確定する最も基本的な配列である。Verilogから引き継がれた構文であり、RTL設計で広く使用される。

**リスト4.1: 静的配列（固定サイズ配列）の宣言と初期化**
...
静的配列は多次元に拡張できる。2次元配列はメモリやレジスタファイルのモデリングでよく使用される。
...
動的配列は、サイズを実行時に決定できる配列である。宣言時にはサイズを指定せず、`new[]`コンストラクタでメモリを確保する。テストベンチにおいて、テストデータの件数が事前に不明な場合に特に有用である。

**リスト4.3: 動的配列の宣言とサイズ確保**
...
動的配列は、ファイルからテストデータを読み込む場面で特に威力を発揮する。

**リスト4.4: ファイルからの動的配列読み込み**
...
連想配列は、スパース（疎）なデータを効率的に格納するためのデータ構造である。通常の配列とは異なり、インデックスが連続している必要がない。巨大なアドレス空間のうち一部のみにデータが存在するメモリモデルなどで威力を発揮する。
...
連想配列の代表的な活用例として、スパースメモリモデルがある。4GBのアドレス空間を持つメモリを静的配列で実現すると膨大なメモリが必要になるが、連想配列を使えばアクセスされたアドレスの分だけメモリを消費する。

**リスト4.6: 連想配列によるスパースメモリモデル**
...
連想配列のインデックス型に`[*]`を使用すると、任意の整数型をキーとして使用できる。

**リスト4.7: ワイルドカードインデックスの使用**
...
連想配列の全要素に対してループ処理を行う場合、`first`、`next`メソッドを使用する。

**リスト4.8: 連想配列のイテレーション**

```systemverilog
// 連想配列のイテレーション
int score_map [string];  // 文字列キーの連想配列
string key;
int value;
...
initial begin
    ...
    // first/next によるイテレーション
    if (score_map.first(key)) begin
        do begin
            value = score_map[key];
            $display("%s: %0d点", key, value);
        end while (score_map.next(key));
    end

    // exists による存在確認
    if (score_map.exists("Alice"))
        $display("Aliceのスコア: %0d", score_map["Alice"]);
end
```

#### 配列間のコピー

動的配列や連想配列のコピーには、単純な代入または`copy`メソッドを使用する。
...
// 配列間のコピー
int src [], dst [];
int assoc_src [int], assoc_dst [int];

initial begin
    // 動的配列のコピー
    src = new[5];
    foreach (src[i]) src[i] = i * 10;

    dst = src;              // シャローコピー（参照のコピー）
    dst = new[5](src);      // ディープコピー（新しい配列を作成）

    // 連想配列のコピー
    assoc_src[0] = 100;
    assoc_src[1] = 200;

    foreach (assoc_src[k])   // 要素ごとのコピー
        assoc_dst[k] = assoc_src[k];
end
```

![図4.2: 静的配列・動的配列・連想配列のメモリ配置の比較](/images/systemverilog-guidebook/ch04_array_memory_layout.drawio.png)

---

### 4.2.4 キュー

キューは、可変長のFIFO（First-In, First-Out）的なデータ構造であり、`[$]`で宣言する。動的配列とは異なり、`new[]`を呼び出す必要がなく、要素の追加と削除が柔軟に行える。テストベンチにおけるトランザクションの管理やスコアボードの実装で頻繁に使用される。
...
キューはUVM等の検証環境でスコアボードを構築する際に頻繁に利用される。以下に簡単な例を示す。

**リスト4.9: キューを用いたスコアボード**
...
![図4.3: キュー（Queue）の操作メソッド一覧](/images/systemverilog-guidebook/ch04_queue_operations.drawio.png)

---

## 4.3 パック配列とアンパック配列

SystemVerilogの配列には、**パック配列**（packed array）と**アンパック配列**（unpacked array）という2つの概念がある。これらはメモリ上の配置が根本的に異なり、適切に使い分けることが重要である。

### 4.3.1 パック配列

パック配列は、全ビットが連続したビットベクタとして格納される。宣言では型名の直後（変数名の前）に次元を記述する。
...
アンパック配列は、各要素が独立したメモリ領域に格納される。宣言では変数名の後ろに次元を記述する。

**リスト4.11: アンパック配列の宣言と操作**
...
パック次元とアンパック次元は同一の変数に混在させることができる。この場合、パック次元が変数名の左側に、アンパック次元が右側に記述される。

**リスト4.12: パック配列とアンパック配列の混在使用**
...
パック配列とアンパック配列の間で代入を行う際は、ストリーミング演算子（`{>>{}}`, `{<<{}}`）を使用する必要がある場合がある。

**リスト4.13: ストリーミング演算子による配列変換**
...
![図4.4: パック配列とアンパック配列のメモリ配置](/images/systemverilog-guidebook/ch04_packed_vs_unpacked.drawio.png)

---

## 4.4 配列メソッド

SystemVerilogの配列には、ソート、検索、集約といった強力な組み込みメソッドが用意されている。これらのメソッドを活用することで、複雑なデータ操作を簡潔に記述できる。

### 4.4.1 ソートと並べ替え

#### sort() と rsort()

`sort()`は昇順に、`rsort()`は降順に配列をソートする。これらのメソッドは配列を直接変更する（in-place操作）。

**リスト4.14: sort()とrsort()によるソート**
...
構造体の配列をソートする場合は、`with`句でソートキーを指定する。
...
`reverse()`は配列の要素を逆順にし、`shuffle()`はランダムに並べ替える。

**リスト4.16: reverse()とshuffle()による並べ替え**
...
配列の検索メソッドは、条件に一致する要素やインデックスをキューとして返す。

#### find()

`with`句で指定した条件に一致する全要素を返す。

**リスト4.17: find()による要素検索**
...
条件に一致する最初の要素または最後の要素を返す。戻り値はキュー（要素数0または1）である。

**リスト4.18: find_first()とfind_last()による検索**
...
**リスト4.21: find_index()によるインデックス検索**
...
集約メソッドは、配列の全要素に対して演算を行い、単一の値を返す。

#### sum() と product()

**リスト4.20: sum()とproduct()による集約演算**
...
`with`句は多くの配列メソッドで使用でき、柔軟な条件指定を可能にする。`item`は配列の各要素を参照する暗黙の変数である。

**リスト4.22: with句による条件指定の活用**
...
![図4.5: 配列操作メソッド比較一覧](/images/systemverilog-guidebook/ch04_array_methods.drawio.png)

---

## 4.5 まとめ

本章では、SystemVerilogの配列と集合について体系的に解説した。以下に重要なポイントを整理する。

**配列の種類**
- **静的配列**はコンパイル時にサイズが確定し、RTL設計で広く使用される。多次元配列にも対応している
- **動的配列**は`new[]`で実行時にサイズを決定でき、`delete()`で解放できる。テストデータの管理に適している
- **連想配列**はスパースなデータ格納に最適であり、`[key_type]`でキーの型を指定する。メモリモデルの構築で威力を発揮する。`[*]`のワイルドカードインデックスも利用可能である
- **キュー**は`[$]`で宣言し、`push_back`/`push_front`/`pop_back`/`pop_front`で柔軟にFIFO的な操作が行える。スコアボード等の検証コンポーネントで頻用される

**パック配列とアンパック配列**
- パック配列は連続したビットベクタとして格納され、ビット全体の操作が可能である。次元は変数名の左側に記述する
- アンパック配列は要素ごとに独立して格納され、任意の型を要素にできる。次元は変数名の右側に記述する
- 両者の混在使用は可能であるが、相互変換にはストリーミング演算子が必要な場合がある

**配列メソッド**
- `sort()`/`rsort()`で昇順・降順ソート、`reverse()`で逆順、`shuffle()`でランダム並べ替えが可能である
- `find()`系メソッドで条件に一致する要素やインデックスを検索できる
- `sum()`/`product()`/`and()`/`or()`/`xor()`で集約演算が行える
- `with`句を使用することで、構造体のフィールドや計算式に基づいた条件指定が可能である

次章（第5章）では、組み合わせ回路と順序回路について解説する。
