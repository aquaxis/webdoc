...
本章では、SystemVerilogにおける**デザインパターンと実践的なコーディング技法**について解説する。ソフトウェア工学で確立されたデザインパターンの考え方をハードウェア設計・検証に適用し、再利用性・保守性・信頼性の高い設計を実現する方法を学ぶ。RTL設計における頻出パターン（FSM、パイプライン、ハンドシェイク、FIFO、アービタ）、検証におけるテストベンチアーキテクチャパターン、再利用可能なコンポーネント設計、コーディングベストプラクティス、よくある落とし穴とアンチパターン、そしてUVM（Universal Verification Methodology）の概要まで、実践的なSystemVerilog開発に必要な知識を体系的に習得する。

![図17.1: デザインパターンの全体像](/images/systemverilog-guidebook/ch17_design_patterns_overview.drawio.png)

---

## 17.2 RTLデザインパターン

RTL設計では、特定の回路構造が繰り返し現れる。これらをパターンとして理解し、正しく実装することで、合成可能で効率的なハードウェアを設計できる。

### 17.2.1 FSM（有限状態マシン）パターン

FSMはデジタル回路で最も基本的なデザインパターンである。推奨される2プロセス方式では、順序回路と組み合わせ回路を明確に分離する。

**リスト17.1: FSMの2プロセスパターン**
...
- **`enum` 型の使用**: 状態名を文字列として扱え、デバッグ性が向上する
- **2プロセス方式**: 順序回路と組み合わせ回路を分離し、見通しを良くする
- **デフォルト値の設定**: `always_comb` 内の先頭でデフォルト値を設定し、ラッチ推論を防止する

![図17.2: FSMの2プロセスパターン](/images/systemverilog-guidebook/ch17_fsm_two_process.drawio.png)

### 17.2.2 パイプラインパターン

パイプラインは、処理をステージに分割してスループットを向上させるパターンである。各ステージ間にレジスタを挿入し、データを1サイクルずつ流す。
...
- **バリッドビットの伝搬**: データと一緒に `valid` 信号をパイプラインに通し、有効性を追跡する
- **パイプラインストール**: バックプレッシャーが必要な場合は `stall` 信号を追加する

### 17.2.3 ハンドシェイクプロトコルパターン

モジュール間のデータ転送を安全に行うために、`valid`/`ready` 方式のハンドシェイクプロトコルを使用する。AXIバスプロトコルでも採用されている標準的な手法である。

**リスト17.3: ハンドシェイクインターフェース**
...
![図17.3: ハンドシェイクプロトコルのタイミング](/images/systemverilog-guidebook/ch17_handshake_timing.drawio.png)

- **valid先行**: 送信側は `ready` を待たずに `valid` をアサートできる
- **転送成立**: `valid && ready` が同時に成立したクロックエッジでデータが転送される
- **デッドロック回避**: `valid` は `ready` に依存してはならないルールが一般的である

### 17.2.4 FIFOパターン

FIFOは異なる速度で動作するモジュール間のデータバッファである。ポインタのMSBでラップアラウンドを検出する手法が一般的である。
...
複数のリクエスタが共有リソースにアクセスする場合、アービタによる調停が必要である。ラウンドロビン方式は全リクエスタに均等な帯域を保証する。
...
検証環境にもデザインパターンがある。テストベンチを体系的に構築することで、再利用性と拡張性が向上する。

### 17.3.1 階層化テストベンチパターン

階層化テストベンチでは、テストベンチを機能ごとのレイヤーに分割する。

![図17.4: 階層化テストベンチの構造](/images/systemverilog-guidebook/ch17_layered_testbench.drawio.png)
...
検証環境の中核を成す3つのコンポーネントである。

**リスト17.6: ドライバ/モニタ/スコアボードパターン**
...
- **ドライバ**: トランザクションレベルの操作を信号レベルに変換してDUTに印加する
- **モニタ**: DUTの出力信号を監視し、トランザクションレベルに再構成する
- **スコアボード**: 期待される出力と実際の出力を比較し、テスト結果を判定する

### 17.3.3 ファクトリパターン

ファクトリパターンを使うと、テストケースごとに異なるトランザクションクラスを動的に切り替えられる。
...
コールバックパターンを使うと、既存のコンポーネントを変更せずに振る舞いを拡張できる。
...
- **拡張性**: 既存のドライバコードを修正せずに新しい振る舞いを追加できる
- **テスト固有のカスタマイズ**: テストケースごとに異なるコールバックを登録できる
...
パラメータを使用することで、同じモジュールをさまざまな構成で再利用できる。

**リスト17.9: パラメータ化レジスタモジュール**
...
パッケージで型、定数、関数を一元管理し、プロジェクト全体で共有する。

**リスト17.10: プロジェクト共通パッケージ**
...
![図17.5: 再利用可能なコンポーネント設計の構造](/images/systemverilog-guidebook/ch17_reusable_components.drawio.png)
...
- **`always_ff`**: 順序回路にのみ使用する。ノンブロッキング代入 `<=` を用いる
- **`always_comb`**: 組み合わせ回路にのみ使用する。ブロッキング代入 `=` を用いる
- **`logic` 型の使用**: `reg` や `wire` の代わりに `logic` を使用し、ドライバ競合を検出する
- **ビット幅の明示**: 定数のビット幅を明示し、意図しないビット拡張を防ぐ
- **ANSIスタイルのポート宣言**: モジュールポートは常にANSIスタイルで記述する
- **名前付きポート接続**: インスタンス接続は位置ベースではなく名前ベースを用いる

**リスト17.11: ポート接続のベストプラクティス**
...
`always_comb` 内ですべての条件分岐に出力値を代入しないと、ラッチが推論される。
...
ブロッキング代入とノンブロッキング代入の不適切な使用はレース条件を引き起こす。
...
リセットで一部のレジスタを初期化し忘れると、シミュレーションと実機で異なる動作を引き起こす。
...
- **マルチドライバ**: 同じ信号を複数の `always` ブロックでドライブしてはならない
- **非同期リセットの同期化忘れ**: 外部リセット信号はダブルフリップフロップで同期化が必要である
- **クロックドメイン交差（CDC）の無視**: 異なるクロックドメイン間にはシンクロナイザが必要である
- **`initial` ブロックでのハードウェア記述**: `initial` は合成不可（テストベンチ専用）である

![図17.6: よくある落とし穴の分類](/images/systemverilog-guidebook/ch17_common_pitfalls.drawio.png)
...
**UVM（Universal Verification Methodology）**は、Accellera Systems Initiative が策定した業界標準の検証方法論である。SystemVerilogのクラスベース検証機能を体系化し、再利用可能な検証環境の構築フレームワークを提供する。

- **標準化されたアーキテクチャ**: コンポーネントの構成と接続方法が統一されている
- **ファクトリメカニズム**: テスト時にオブジェクトの型を差し替えられる
- **フェーズ機構**: `build_phase`, `connect_phase`, `run_phase` 等のライフサイクルが定義されている
- **シーケンスメカニズム**: テストシナリオをシーケンスとして構造化し再利用できる
- **TLM（Transaction Level Modeling）**: コンポーネント間通信をトランザクションレベルで抽象化する

![図17.7: UVMアーキテクチャ](/images/systemverilog-guidebook/ch17_uvm_architecture.drawio.png)
...
本章では、SystemVerilogにおけるデザインパターンと実践的なコーディング技法について学んだ。主要なポイントを振り返る。

- **RTLデザインパターン**: FSM（2プロセス方式）、パイプライン、ハンドシェイクプロトコル、FIFO、アービタなど、合成可能な回路の定番パターンを理解した。これらを正しく実装することで、信頼性の高いハードウェアを設計できる
- **検証デザインパターン**: 階層化テストベンチ、ドライバ/モニタ/スコアボード、ファクトリパターン、コールバックパターンなど、検証環境のアーキテクチャパターンを学んだ。これらにより再利用性と拡張性の高い検証環境を構築できる
- **再利用可能なコンポーネント設計**: パラメータ化モジュール、インターフェース、パッケージを活用した再利用性の高い設計手法を学んだ
- **コーディングベストプラクティス**: 命名規則、代入方式の使い分け、`always_ff`/`always_comb` の正しい使用など、保守性の高いコードを書くためのガイドラインを確認した
- **よくある落とし穴**: ラッチ推論、レース条件、リセット漏れ、マルチドライバなど、頻出するバグのパターンとその回避方法を理解した
- **UVM概要**: UVMのアーキテクチャ、フェーズ機構、基本コンポーネントの概念を学んだ。UVMは業界標準の検証フレームワークであり、大規模プロジェクトの検証生産性を大幅に向上させる

デザインパターンは「正解」ではなく「道具」である。プロジェクトの要件、チームの規模、設計の複雑さに応じて、適切なパターンを選択・組み合わせることが重要である。パターンの本質を理解し、各パターンが解決する問題を意識して使い分けよう。

次の第17章では、**SystemVerilogプロジェクト演習**として、本書で学んだ知識を総合的に活用する実践的なプロジェクトに取り組む。RTL設計から検証環境の構築まで、一貫した開発フローを体験する。
