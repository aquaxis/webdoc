...
本章では、これまでに学んだSystemVerilogの知識を統合し、**実践的なプロジェクト演習**に取り組む。3つのプロジェクト――UARTコントローラ、簡易CPU、SPIマスターコントローラ――を通じて、RTL設計から検証までの一連のフローを体験する。各プロジェクトでは仕様策定、RTLコーディング、テストベンチ作成、機能カバレッジの収集、デバッグまでを一貫して実施する。個々の章で学んだ技術がどのように組み合わさるかを実感し、実務に近い設計・検証スキルを身につけよう。

![図18.1: プロジェクト演習の全体構成](/images/systemverilog-guidebook/ch18_project_overview.drawio.png)
...
各プロジェクトは以下のフローに従って進める。

1. **仕様策定**: 機能仕様を明確化し、インターフェースを定義する
2. **アーキテクチャ設計**: モジュール分割とデータパスを決定する
3. **RTLコーディング**: SystemVerilogで各モジュールを実装する
4. **テストベンチ作成**: 制約付きランダムテストとダイレクトテストを構築する
5. **シミュレーション実行**: テストを実行し、波形とログで動作を確認する
6. **カバレッジ収集・分析**: 機能カバレッジとコードカバレッジの達成率を確認する
7. **デバッグ・修正**: 不具合を発見した場合は原因を特定し修正する

![図18.2: 設計・検証フロー](/images/systemverilog-guidebook/ch18_design_verification_flow.drawio.png)

### 18.2.2 検証目標の設定

プロジェクト演習では、表18.1のカバレッジ目標を設定する。
...
**UART（Universal Asynchronous Receiver/Transmitter）**は、非同期シリアル通信の標準的なインターフェースである。本プロジェクトでは以下の仕様でUARTコントローラを設計する。
...
![図18.3: UARTフレームフォーマット](/images/systemverilog-guidebook/ch18_uart_frame_format.drawio.png)

UARTフレームは表18.2の構成である。
...
![図18.4: UARTモジュール構成](/images/systemverilog-guidebook/ch18_uart_module_structure.drawio.png)

### 18.3.3 ボーレートジェネレータ

ボーレートジェネレータは、システムクロックを分周してUART通信のタイミング信号を生成する。
...
送信モジュールは、パラレルデータをシリアルに変換して出力する。
...
受信モジュールは、シリアル入力をパラレルデータに変換する。16倍オーバーサンプリングにより、ビットの中央でデータをサンプリングする。
...
制約付きランダムテストを用いたテストベンチである。送信データをランダムに生成し、ループバック接続（TX出力をRX入力に接続）で正しく受信できることを確認する。
...
本プロジェクトでは、8ビットの簡易CPUを設計する。教育目的のシンプルなアーキテクチャであるが、フェッチ・デコード・実行の基本的なパイプライン概念を含む。
...
![図18.5: 簡易CPU命令フォーマット](/images/systemverilog-guidebook/ch18_cpu_instruction_format.drawio.png)
...
![図18.6: 簡易CPUブロック図](/images/systemverilog-guidebook/ch18_cpu_block_diagram.drawio.png)
...
テストベンチでは、命令メモリにプログラムをロードし、期待される実行結果と比較する。
...
**SPI（Serial Peripheral Interface）**は、マスター/スレーブ方式の同期式シリアル通信インターフェースである。4本の信号線で全二重通信を行う。
...
![図18.7: SPIバス接続図](/images/systemverilog-guidebook/ch18_spi_bus_connection.drawio.png)

本プロジェクトでは以下の仕様でSPIマスターを設計する。
...
テストベンチで使用するスレーブモデルである。マスターからの送信データをエコーバックする。
...
4つのSPIモードすべてをテストし、全二重通信の正しさを確認する。
...
プロジェクト演習でバグに遭遇した場合、以下の体系的なアプローチで原因を特定する。これは第15章で学んだデバッグ技法の実践的な適用である。

1. **症状の特定**: テストの失敗メッセージやアサーション違反を正確に把握する
2. **再現性の確認**: 同じ条件で必ず再現するか、ランダムシードに依存するかを確認する
3. **問題の切り分け**: 関連するモジュールを特定し、影響範囲を絞り込む
4. **波形の確認**: 信号のタイミングと値を波形ビューワで確認する
5. **根本原因の特定**: RTLのロジックやステートマシンの誤りを発見する
6. **修正と回帰テスト**: 修正後、既存のテストがすべて通ることを確認する

![図18.8: デバッグフロー](/images/systemverilog-guidebook/ch18_debug_flow.drawio.png)

### 18.6.2 各プロジェクトでの典型的な問題

プロジェクトごとに起こりやすい典型的な問題と、その対処法を以下にまとめる。

**UARTコントローラ**

**表18.8: UARTコントローラの典型的な問題**
...
各プロジェクトに組み込むべきアサーションの例を示す。アサーションを事前に埋め込んでおくことで、バグの早期発見が可能になる。

**リスト18.13: UARTアサーション例**
...
各プロジェクトに取り組む際は、以下のステップに従うことを推奨する。

1. **仕様書を読む**: まず仕様を完全に理解してから設計を開始すること
2. **ブロック図を描く**: モジュール間のインターフェースを明確にする
3. **段階的に実装する**: 最小限の機能から始めて、徐々に機能を追加する
4. **こまめにテストする**: モジュール単位でテストし、統合前にバグを発見する
5. **波形を確認する**: 期待通りの動作かどうか、必ず波形で確認する

### 18.7.2 検証計画の作成

各プロジェクトで検証計画を作成することが重要である。表18.11のテンプレートを参考にされたい。
...
プロジェクト演習では、カバレッジ駆動検証（CDV: Coverage Driven Verification）のアプローチを採用する。
...
各プロジェクトの基本演習を完了した後、以下の発展課題にも挑戦されたい。

**UART発展課題**

- **パリティ機能の追加**: 偶数パリティ/奇数パリティの送受信を実装する
- **FIFO統合**: 8段のFIFOを送信/受信パスに組み込む
- **フロー制御**: RTS/CTSによるハードウェアフロー制御を追加する

**簡易CPU発展課題**

- **パイプライン化**: 2段または3段のパイプラインを実装する
- **命令追加**: シフト演算、ジャンプ命令を追加する
- **割り込み処理**: 単純な割り込みメカニズムを追加する

**SPI発展課題**

- **マルチスレーブ対応**: 複数のCS信号を管理するデコーダを追加する
- **DMA連携**: DMAコントローラとの連携インターフェースを設計する
- **可変データ幅**: 8/16/32ビットの転送幅を動的に切り替え可能にする

---

## 18.8 まとめ

本章では、3つのプロジェクト演習を通じて、SystemVerilogによる設計・検証の実践スキルを学んだ。

1. **UARTコントローラ**では、ボーレートジェネレータ・送信モジュール・受信モジュールの設計と、ループバック方式による検証手法を学んだ。16倍オーバーサンプリングによるビット中央サンプリングが信頼性の鍵となる。
2. **簡易CPU**では、命令セットアーキテクチャの定義からALU・レジスタファイル・制御ユニットの実装まで、プロセッサ設計の基本フローを体験した。命令デコードと制御信号の正確な対応付けが重要である。
3. **SPIマスターコントローラ**では、4つのSPIモード（CPOL/CPHAの組み合わせ）に対応した設計と、スレーブモデルを用いた全二重通信の検証を行った。
4. **デバッグ戦略**として、体系的なアプローチ（症状特定→再現性確認→問題切り分け→波形確認→根本原因特定→回帰テスト）を各プロジェクトに適用する手法を学んだ。
5. **アサーションの埋め込み**により、プロトコル違反やタイミング異常を早期に検出できることを確認した。
6. **カバレッジ駆動検証（CDV）**のアプローチにより、検証の完了基準を定量的に管理する方法を実践した。

次章では、SystemVerilogのさらなる学習に向けたリソースと、本書全体のまとめを行う。
